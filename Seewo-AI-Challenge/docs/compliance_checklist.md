...
| LLM API 调用数据最小化 | ⚠️ | 当前批改请求含 student_id，建议改为匿名 token；订正文本不含学生姓名 |
...
| LLM 请求日志 | ⚠️ | 订正批改调 LLM 时，请求体含订正文本（学生手写内容）；建议日志只记录 request_id + latency，不记录 payload |
...
| 家长知情同意机制 | ❌ | 未实现；建议首次登录时弹出《未成年人个人信息处理告知》+ 家长确认 |
| 数据删除权 | ❌ | 未实现"删除我的数据"功能；建议管理员后台提供数据导出+删除 |
...
| 优先级 | 事项 | 负责人 | 截止 |
|--------|------|--------|------|
| P0 | 家长知情同意机制（首次登录弹窗） | 全栈开发工程师 | Sprint 4 |
| P0 | LLM API 调用 student_id 匿名化 | 全栈代码审查官 | Sprint 4 |
| P1 | 数据删除/导出功能（管理员后台） | 快速原型师 | Sprint 4 |
